...
当前模拟器停在“共享屏幕”页面。由用户自行点击“启动服务”，确认系统录屏授权；如果系统要求选择范围，测试完整设备画面时选择整个屏幕。然后开启“输入控制”，按 Android 提示在无障碍设置中允许 `Remote Control POC Input`。

录屏会让受控端画面可见，无障碍输入服务可操作其他应用；不能通过脚本自动授予或绕过权限提示。电脑操作技能要求把此步骤交还用户。本轮没有代替用户同意授权，也没有启用屏幕共享服务。

后续先测试点击、滑动、返回、文本输入、横竖屏坐标和停止授权后的行为，再安排两端网络与画质测试。初始功能测试无需同时启动第二台模拟器。
...
本轮仅新增、更新验证文档，未修改应用源码、签名、包名、编解码路径或既有安装脚本。设备侧变更仅为启动上述现有模拟器并安装、打开独立 POC 应用；原有应用和数据未删除。
